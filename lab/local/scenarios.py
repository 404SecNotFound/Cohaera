...
"""The five states of one workflow, as telemetry a collector would have written.
...
def _call(sid: str, ts: float, seq: int, tool: str, span: str, *,
          ...
          markers: bool = False, receipt: dict | None = None) -> list[dict]:
    ...
    start = _ev(sid, ts, "tool_start", seq=seq, tool=tool, span=span,
                data={"action": "invoke_tool", "tool_args": args,
                      "arg_digest": arg_digest(args), "reversible": not ok})
