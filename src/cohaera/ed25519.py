...
def _is_small_order(p: tuple[int, int, int, int]) -> bool:
    """Is ``p`` in the order-8 torsion subgroup? ``[8]p == identity``.

    THE FORGERY THIS CLOSES. Verification checks ``[s]G == R + [k]A``. Give it
    the identity point as A, the identity point as R and ``S = 0`` and both
    sides are the identity, for EVERY message -- one "signature" that verifies
    anything, under a key that verified nothing. Three of the eight small-order
    encodings reached that outcome here.

    RFC 8032 does not require this check and every serious implementation makes
    it anyway, libsodium included, because the alternative is that a key which
    cannot sign can still verify. It costs three point doublings, which is
    nothing beside the two scalar multiplications either side of it.

    Applied to R as well as A. A legitimate R is ``[r]G`` for a hash-derived r,
    so it lands in the torsion subgroup with probability about 2^-252; refusing
    it costs no honest signature and removes the family of tricks that add a
    small-order component to an otherwise valid point.
    """
    q = _add(p, p)
    q = _add(q, q)
    q = _add(q, q)
...
def admissible_public_key(public_key: bytes) -> bool:
    """Is this a key anybody could have generated, rather than merely bytes?

    Three conditions, and the third alone is not enough: ``[L]A == identity``
    holds for the IDENTITY point too, because the identity multiplied by
    ...
    ``[L]A == identity``. Strictly stronger than :func:`_is_small_order`, and
    strictly more expensive -- a full scalar multiplication -- which is why the
    two are used in different places.

    ``verify`` cannot afford this per signature and does not need it: the
    small-order rejection there closes the forgery, and a MIXED-order key (a
    valid point plus a torsion component) cannot produce a signature that
    verifies under it without the private scalar. Key ADMISSION can afford it
    and should make it, because a key that is not in the prime-order subgroup
    is not a key anybody generated by the standard procedure, and the honest
    thing for an operator's trust store to do with one is refuse it by name
    rather than carry it and hope.
    """
    if not isinstance(public_key, (bytes, bytearray)) or len(public_key) != KEY_BYTES:
        return False
    point = _decompress(bytes(public_key))
    if point is None:
        return False                      # not on the curve, or non-canonical
    if _is_small_order(point):
        return False                      # identity and the order-2/4/8 points
    return _equal(_mul_var(point, L), _IDENTITY)
...
def verify(public_key: bytes, message: bytes, signature: bytes) -> bool:
    ...
    if a_point is None:
        return False
    if _is_small_order(a_point):
        # A key of order 8 or less cannot sign, and without this it can still
        # VERIFY -- identity A, identity R and S = 0 satisfy the equation for
        # every message. See _is_small_order.
        return False
