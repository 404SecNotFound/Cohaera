...
# ---------------------------------------------------------------------------
# The documentation map. Sixteen files is enough to get lost in.
# ---------------------------------------------------------------------------
...
def test_the_documentation_map_indexes_every_document():
    """A map that misses a document is worse than no map: a reader who trusts
    it concludes the missing file does not exist.

    Exemptions are narrow and named. Generated files and the map itself are
    out; everything a human wrote is in.
    """
    exempt = {
        "README.md",                 # the map, and the root README it links
        "EVALUATION-CARD.md",        # generated by eval/run_eval.py
        "RESULTS.md",                # generated by lab/local/run.py
        "NOTICE", "LICENSE",
        # Forms rather than documents: GitHub renders them into a UI, nobody
        # reads them as prose, and indexing them would train a reader to
        # ignore rows in this map.
        "pull_request_template.md",
    }
    listed = DOC_MAP.read_text(encoding="utf-8")
    missing = []
    for path in _tracked_markdown_paths():
        if path.name in exempt:
            continue
        rel = path.relative_to(REPO)
        # Either the filename or the path appears somewhere in the map.
        if path.stem not in listed and str(rel) not in listed:
            missing.append(str(rel))
    assert not missing, (
        "docs/README.md does not index these, so a reader following it would "
        f"conclude they do not exist: {sorted(missing)}")


def test_every_link_in_the_documentation_map_resolves():
    """A map of dead links is a map that costs more than it saves."""
    text = DOC_MAP.read_text(encoding="utf-8")
    broken = []
    for target in re.findall(r"\]\((?!https?:)([^)#]+)", text):
        if not (DOC_MAP.parent / target).resolve().exists():
            broken.append(target)
    assert not broken, f"docs/README.md links nothing: {sorted(set(broken))}"


def test_the_readme_leads_with_the_honest_false_positive_number():
    """R-20's last corner. The README published false positives per 1000
    SESSIONS -- a population that is a third attacks, because this corpus needs
    enough of them to measure -- while the evaluation card said in as many
    words never to plan against it. Derived and checked, and 136 lower than the
    honest figure, so nothing ever flagged it.

    A checker enforcing the wrong number is worse than no checker: it converts
    a mistake into a guarantee.
    """
    readme = (REPO / "README.md").read_text(encoding="utf-8")
    assert "per 1000 **benign** sessions" in readme
    assert "| false positives per 1000 sessions |" not in readme, (
